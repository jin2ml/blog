...
layui.define('jquery', function (exports) {
    ...
    var enterSkin = 'layui-atree-enter',
        Atree = function (options) {
            ...
            this.userKey = this.props.user || props.user;
    ...
    Atree.prototype.getDom = function (item) {
        ...
        //自定义前置图标
        var myicon = '<i class="layui-icon layui-icon-add-1"></i>';
        if (item.user) {
            myicon = '<i class="layui-icon layui-icon-friends"></i>';
        }
        return {
            spread: function () {
            ...
            node: function () {
                return myicon + '<a href="' + (item.href || 'javascript:;') + '" ' + (
    ...
    Atree.prototype.checkbox = function (elem, item) {
        ...
        var setCheck = function (elem, item, type) {
            ...
            if (item) {
                ...
                //当树形图不同结点有多个相同元素，重复点击另外一个相同元素整个数组会被推出干净
                //新的if条件会把重复结点都推入数组
                //解决策略，在应用处增加一个去重数组的方法
